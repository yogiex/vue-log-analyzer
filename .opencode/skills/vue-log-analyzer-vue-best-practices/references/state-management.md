...
**Impact: HIGH** - Use the lightest state solution that fits your app architecture.
...
- **Pinia:** SSR/Nuxt apps, medium-to-large apps, and cases requiring DevTools.
...
export const cart = reactive({ items: [] as Array<{ id: string; qty: number }> })
```
...
function createCartStore() {
  const state = reactive({ items: [] as Array<{ id: string; qty: number }> })
...
export function useServerCart() { return cartStore }
```
...
export const useCartStore = defineStore('cart', {
  state: () => ({ items: [] as Array<{ id: string; qty: number }> }),
  actions: { addItem(id: string, qty = 1) {
...
export const useAuthState = createGlobalState(() => {
  ...
  function setToken(next: string | null) { token.value = next }
