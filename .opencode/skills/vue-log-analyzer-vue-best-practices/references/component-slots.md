...
type: best-practice
tags: [vue3, slot, component, template, typescript]
---
...
<template>
  <article class="card">
    ...
    <section class="card-body"><slot /></section>
...
<template>
  <article class="card">
    ...
    <section v-if="$slots.default" class="card-body"><slot /></section>
...
```vue
<template>
  <button type="submit" class="btn-primary"><slot /></button>
</template>
```
...
```vue
<template>
  <button type="submit" class="btn-primary"><slot>Submit</slot></button>
</template>
```
...
function onMove(event: MouseEvent) { x.value = event.pageX; y.value = event.pageY }
