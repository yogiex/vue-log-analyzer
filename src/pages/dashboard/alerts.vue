    ...
    <v-layout class="rounded rounded-md" style="min-height: 100vh;">
        ...
        <v-main class="align-center justify-center" style="min-height: 500px;">
            <v-container>
                <v-row>
                    <v-col>
                        <v-text-field label="Label" prepend-icon="$vuetify" variant="outlined"></v-text-field>
                    </v-col>
                </v-row>
            </v-container>
            <v-container>
                <v-row>
                    ...
                    <v-col>
                        <v-select label="Status" :items="['Open', 'Closed', 'On Progress']" variant="outlined"></v-select>
                    </v-col>
                </v-row>
            </v-container>
            <v-data-table :headers="headers" :items="dataPeserta" :search="search">
                <template v-slot:item.status="{ item }">
                    <div class="text-center">
                        <v-chip :color="dataPeserta.status == 'terindikasi' ? 'green' : 'red'"
                            :text="dataPeserta.status == 'terindikasi' ? 'Aman' : 'Terindikasi'" class="text-uppercase"
                ...
                <template v-slot:item.track_progress="{ item }">
                    ...
                    <div class="text-end">
                        <v-select :label="Select" :items="dataPeserta.track_progress == 'open' ? ['OPEN'] : ['CLOSED']"
